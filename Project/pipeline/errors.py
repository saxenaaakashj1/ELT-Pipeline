    ...
    errors: dict[str, dict[type[Exception], str]] = {
        "file": {
            FileNotFoundError: (
                ...
                f"{Fore.CYAN}'{file_name}'{Fore.RED} was not found. "
                f"{Style.RESET_ALL}\n"
                f"{Fore.GREEN}👉 Action: Verify the file path and ensure the "
                f"file {Fore.CYAN}'{file_name}'{Fore.GREEN} exists."
            ),
            PermissionError: (
                f"\n{Fore.RED}🚫 Error: Insufficient permissions to access "
                f"the file {Fore.CYAN}'{file_name}'{Fore.RED}.{Style.RESET_ALL}\n"
        ...
        "pandas": {
            pd.errors.EmptyDataError: (
                f"\n{Fore.RED}🚫 Error: The file {Fore.CYAN}'{file_name}'{Fore.RED}"
...
def handle_mysql_errors(e: mysql.connector.Error, is_db_error: bool = False) -> None:
    """
