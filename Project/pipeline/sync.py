...
def sync_data(source: pd.DataFrame, destination: MySQLConnection, file: str) -> None:
    """
...
def infer_primary_key(source: pd.DataFrame) -> Optional[str]:
    ...
    numeric_columns = [
        col for col in unique_columns if pd.api.types.is_numeric_dtype(source[col])
    ]
    if numeric_columns:
...
def create_table_from_dataframe(
    ...
    columns: list[str] = [
        f"{sanitize_column_name(str(col))} "
...
def upsert_data(cursor, source: pd.DataFrame, table_name: str) -> None:
    ...
    """
    column_names: str = ", ".join([sanitize_column_name(col) for col in source.columns])
    ...
    data_to_insert: list[tuple] = [tuple(row) for row in source.itertuples(index=False)]
...
def perform_soft_delete(
    ...
    temp_table_create_query = (
        f"CREATE TEMPORARY TABLE {temp_table_name} (" f"{primary_key} VARCHAR(255)" ");"
    )
